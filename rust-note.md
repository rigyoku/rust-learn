...
* 元组结构体
    ...
    * 需要使用分号来结尾
    * 结构时, 需要使用一个元组结构体来接受 `let Tuple (t2) = t1;`
...
* 可以通过for循环遍历迭代器, 会获得迭代器所有权
* 迭代器都实现了`Iterator`的`trait`, 定义关联类型`Item`和`next`方法获取元素
    * 可以队迭代器调用`next`方法获取元素, 获取到`None`表示遍历到头了
    * 需要是可变迭代器
* 迭代适配器(比如map), 对每个元素进行操作(执行闭包), 返回一个新的迭代器, 每个元素都是修改过的元素
    * 返回的适配器如果不消费就没用, 因为他也是惰性的
